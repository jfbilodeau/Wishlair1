...
void Runtime::registerDefaultInstructions() {
    ...
    registerInstruction(OpCodes::op_return, op_return, "Return from the current script. Does not free variables from the stack", {});
    registerInstruction(
    ...
    // Flow control
    registerInstruction(OpCodes::op_jump, op_jump, "Jump to the given index (address)", {Operand::Integer});
    registerInstruction(
    ...
    registerInstruction(OpCodes::op_pop_n, op_pop_n, "Pop (free) `n` values from the stack", {Operand::Integer});
    ...
    registerInstruction(OpCodes::op_call_command, op_call_command, "Call the given command id", {Operand::Command});
    ...
    // ID operators
    registerInstruction(OpCodes::op_id_push, op_id_push, "Push the `id` value to the stack", {Operand::Id});
